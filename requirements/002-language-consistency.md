...
| FR-2.3: Inline/Block Conflict Validation | ✅ Implemented | Warnings for inline vs block conflicts; inline precedence |
| FR-2.4: ESLint Configuration | ✅ Implemented | Strict rules configured; **0 errors**, warnings limited to tests; autogenerated syntaxes excluded |
...
### ✅ FR-2.3: Inline/Block Conflict Validation [IMPLEMENTED]

**Status**: **IMPLEMENTED** - Validation now warns when the same property is specified inline and in a documentation block, with inline values taking precedence.

**Implemented Behavior**:
- Detects `as` (inline role) vs `role:` (block) conflicts
- Detects `by` (inline team) vs `team:` (block) conflicts
- Inline value takes precedence; warning suggests using one form for clarity
- Grammar does not define a block domain assignment; `for` vs block domain conflict is not applicable

**Validation Location**: `dsl/domain-lang/packages/language/src/validation/bounded-context.ts`

**Example Warning Message**:
```text
Role specified both inline ('as CoreRole') and in block ('role: SupportingRole'). Inline value takes precedence. Consider using only one form for clarity.
```
...
3. Multiple conflicts simultaneously
4. No conflict when only one form used
...
- Rules enforced:
  ...
  - Disabled core `no-useless-escape` to avoid false positives in autogenerated Monarch syntax files
...
- **0 errors** (autogen grammar lint conflicts resolved)
- **Warnings limited to test code** (missing return types, non-null assertions for setup)
...
**Then** no error prevents parsing
...
- [x] Implement inline/block conflict validation (FR-2.3)
- [x] Add comprehensive test suite for new validations
- [ ] Ensure 80%+ coverage maintained

### Phase 3: Tooling (1 week)
 
- [x] Configure ESLint strict rules (FR-2.4)
- [x] Fix existing linting violations (disabled `no-useless-escape` for autogen code)
- [x] Add coverage thresholds to vitest config
