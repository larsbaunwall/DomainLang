...
#### Block 19 (line 300): Alternative Block Names
**Issue**: Uses `...` placeholder syntax which is invalid
**Fix**: Replace with real terminology example

#### Block 33 (line 528): Decisions with Classification Categories  
**Issue**: Standalone `decisions` block not inside a bounded context
**Fix**: Wrap in `bc ... { decisions { ... } }`

#### Block 39 (line 640): Import with Alias - Qualified Name Issue
**Issue**: References `SharedTypes.CoreDomain` but classification isn't imported properly
**Fix**: Remove qualified name reference or restructure example

#### Blocks 47-48 (lines 746-770): Large E-Commerce Example
**Issue**: Uses qualified names like `StrategicClassifications.CoreDomain` in Domain classification property, but this syntax isn't supported. Domain.classification expects a direct reference, not a qualified name.
**Fix**: Remove qualified name syntax or adjust to use direct references
