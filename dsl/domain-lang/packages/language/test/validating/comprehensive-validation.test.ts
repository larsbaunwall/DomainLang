...
describe('Validation Tests', () => {
    ...
    describe('Context Group Validation', () => {
        test('warns when context group has no contexts', async () => {
            const document = await testServices.parse(s`
                Domain Sales {
                    vision: "Handle all sales activities"
                }
                ContextGroup EmptyGroup for Sales {
                    role: Core
                    // No contains clause
                }
            `);
            ...
            expectValidationWarnings(document, [
                "ContextGroup 'EmptyGroup' contains no bounded contexts"
            ]);
        });

        test('warns when context group contains contexts from multiple domains', async () => {
            const document = await testServices.parse(s`
                Domain Sales {
                    vision: "Handle all sales activities"
                }
                Domain Marketing {
                    vision: "Handle all marketing activities"
                }
                
                BoundedContext SalesBC for Sales {
                    description: "Sales context"
                }
                BoundedContext MarketingBC for Marketing {
                    description: "Marketing context"
                }
                
                ContextGroup MixedGroup {
                    contains SalesBC, MarketingBC
                }
            `);

            expectValidationWarnings(document, [
                "contains bounded contexts from multiple domains"
            ]);
        });

        test('accepts context group with contexts from same domain', async () => {
            const document = await testServices.parse(s`
                Domain Sales {}
                Classification Core
                
                BoundedContext SalesBC1 for Sales
                BoundedContext SalesBC2 for Sales
                
                ContextGroup SalesGroup for Sales {
                    role: Core
                    contains SalesBC1, SalesBC2
                }
            `);
        ...
        test('should validate role classifier reference', async () => {
            const document = await testServices.parse(s`
                Domain Sales {}
                BoundedContext SalesBC for Sales
                
                ContextGroup TestGroup for Sales {
