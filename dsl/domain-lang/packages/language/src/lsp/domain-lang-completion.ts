...
export class DomainLangCompletionProvider extends DefaultCompletionProvider {
    ...
    // Check if we're at the model/namespace level
        const node = context.node;
    return ast.isModel(node) || ast.isNamespaceDeclaration(node);
    }
    ...
    private addTopLevelSnippets(acceptor: CompletionAcceptor, context: CompletionContext): void {
        ...
        acceptor(context, {
            ...
            kind: CompletionItemKind.Snippet,
            insertText: [
                'namespace ${1:qualified.name} {',
                '\t$0',
                '}',
                ''].join('\n'),
            insertTextFormat: InsertTextFormat.Snippet,
            documentation: 'Namespace declaration for hierarchical organization',
