...
import { DomainLangNameProvider } from './domain-lang-naming';
...
export class DomainLangScopeComputation extends DefaultScopeComputation {
            ...
            if (isType(modelNode)) {
                ...
                if (name) {
                    if (isPackageDeclaration(modelNode.$container)) {
                        name = (this.nameProvider as DomainLangNameProvider).getQualifiedName(modelNode.$container as PackageDeclaration, name);
                    }
    ...
    protected createQualifiedDescription(pack: PackageDeclaration, description: AstNodeDescription, document: LangiumDocument): AstNodeDescription {
        const name = (this.nameProvider as DomainLangNameProvider).getQualifiedName(pack.name, description.name);
