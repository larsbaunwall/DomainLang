...
export class DomainLangRenameProvider extends DefaultRenameProvider {
    ...
    protected buildQualifiedName(node: AstNode): string | undefined {
        ...
        if (name) {
            if (isPackageDeclaration(node.$container)) {
                name = (this.nameProvider as DomainLangNameProvider).getQualifiedName(node.$container, name);
            }
        }
        return name;
    }
